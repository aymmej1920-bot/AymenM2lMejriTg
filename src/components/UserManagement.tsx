import React from 'react';
    ...
    // import UserRoleEditDialog from './user-management/UserRoleEditDialog'; // Removed import
    ...
    interface UserManagementProps {
      // onUpdateUserRole: (userId: string, newRole: UserRole) => Promise<void>; // Removed prop
    }

    const UserManagement: React.FC<UserManagementProps> = () => { // Removed onUpdateUserRole prop
      const {
        loading,
        error,
      ...
      } = useUserManagement(); // Removed onUpdateUserRole from hook call
      ...
      const handleExportUsers = (usersToExport: Profile[]) => { // Type usersToExport
        const dataToExport = usersToExport.map(user => ({
          ...
          Email: user.email,
          // Rôle: user.role, // Removed role
          "Date Dernière Mise à Jour": new Date(user.updated_at).toLocaleDateString(),
        }));

        // Update headers as role is removed
        const headers = ["Nom", "Email", "Date Dernière Mise à Jour"];
      ...
      return (
        <div className="space-y-6 animate-fade-in">
          ...
          <UserManagementTable
            ...
            // onEditRole={handleEditRole} // Removed
            onDeleteUser={confirmDeleteUser}
            // canEditRole={canEditRole} // Removed
            canDeleteUser={canDeleteUser}
          />

          {/* UserRoleEditDialog is removed */}
          {/* <UserRoleEditDialog
