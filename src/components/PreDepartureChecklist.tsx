...
const PreDepartureChecklistComponent: React.FC<PreDepartureChecklistProps> = ({ data, onAdd }) => { // 'userRole' removed from destructuring
  ...
  // const canAdd = true; // All authenticated users can add - Removed
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    // No need for canAdd check here, as the button is always visible
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      required
                      // readOnly={!canAdd} // Removed readOnly prop
                    />
                  </div>
                  <div>
                    ...
                    <select
                      ...
                      className="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-blue-500 focus:border-blue-500"
                      required
                      // disabled={!canAdd} // Removed disabled prop
                    >
                ...
                <div className="flex justify-end space-x-4 mt-8">
                  ...
                  {/* canAdd replaced with true */}
                    <button
                      type="submit"
                      className="px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-all<dyad-problem-report summary="1 problems">
<problem file="src/components/PreDepartureChecklist.tsx" line="12" column="87" code="6133">'userRole' is declared but its value is never read.</problem>
</dyad-problem-report><think>**Fixing the Compiler Error**

I've got a TypeScript compile-time error to address in `src/components/PreDepartureChecklist.tsx`.  The complaint is about the declared, but unused `userRole` variable. I'm focusing on the component's structure to determine the simplest solution, likely removing or revising the `userRole` destructuring.


**Resolving the Unused Variable**

I'm focused on the `PreDepartureChecklist.tsx` file now.  It's clear I missed a spot during the recent code cleanup. My current priority is eliminating the unused `userRole` variable from the destructuring. I'll need to remove it and the dependent code, like the `canAdd` variable, and its conditional alert logic. This should clean up the last of the cruft.


</think>Vous avez tout à fait raison ! Il semble que j'aie manqué de supprimer la prop `userRole` de la déstructuration dans le composant `PreDepartureChecklistComponent` lors de la dernière mise à jour.

Pour corriger cette erreur et finaliser la suppression des vérifications de rôle dans ce composant :

1.  Je vais retirer `userRole` de la déstructuration des props du composant `PreDepartureChecklistComponent`.
2.  Je vais supprimer la variable `canAdd` car elle est maintenant toujours `true`.
3.  Je vais retirer la condition `(userRole === 'admin' || userRole === 'direction')` de l'alerte pour les checklists mensuelles manquantes, car cette alerte doit être visible pour tous les utilisateurs.

Voici le fichier mis à jour :

<dyad-write path="src/components/PreDepartureChecklist.tsx" description="Removing unused 'userRole' prop and role-based variables.">
import React, { useState } from 'react';
import { Plus, CheckCircle, XCircle, AlertTriangle } from 'lucide-react';
import { FleetData, PreDepartureChecklist } from '../types';
import { showSuccess, showError } from '../utils/toast';
