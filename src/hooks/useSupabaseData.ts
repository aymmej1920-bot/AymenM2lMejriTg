import { useState, useEffect, useCallback } from 'react';
    ...
    export const useSupabaseData = <T>(
      ...
      const fetchData = useCallback(async () => {
        console.log(`[useSupabaseData] Attempting to fetch data for table: ${tableName}. Enabled: ${enabled}. Current User ID: ${currentUser?.id}. Session active: ${!!session}. Skip User ID Filter option: ${skipUserIdFilter}`);
        ...
        try {
          ...
          } else if (skipUserIdFilter) { // Simplified: if skipUserIdFilter is true, no user_id filter
            console.log(`[useSupabaseData] Skipping user_id filter for ${tableName}.`);
          } else {
      ...
      }, [session?.user, currentUser?.id, tableName, enabled, filters, skipUserIdFilter, page, pageSize, sortBy]);
