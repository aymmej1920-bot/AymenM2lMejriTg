import { useEffect } from 'react';
    ...
    import { Truck, Users, Route as RouteIcon, Fuel, FileText, Wrench, BarChart3, LogOut, ClipboardCheck, FileText as ReportIcon, UserCog, User as UserIcon, CalendarDays, Bell } from 'lucide-react'; // Removed CalendarClock, ShieldCheck
    ...
    // import PermissionsOverview from './components/PermissionsOverview'; // Removed import
    import ProtectedRoute from './components/ProtectedRoute';
    import CalendarPage from './pages/CalendarPage';
    import NotificationCenter from './components/NotificationCenter';
    import { Resource, Action, OperationResult } from './types'; // Removed UserRole
    ...
    export default function App() {
      ...
      // Simplified loading state as permissions are removed
      const appIsLoading = isLoading || isProfileLoading;
      ...
      }, [session, appIsLoading, navigate, location.pathname]);
      ...
      // handleUpdateUserRole is no longer needed as roles are removed
      ...
      // userRole is no longer relevant
      ...
      const tabs = [
        ...
        // User management is now accessible to all authenticated users
        { id: 'user-management', name: 'Gestion Utilisateurs', icon: UserCog, path: '/user-management' },
        // Permissions overview is removed
        // { id: 'permissions-overview', name: 'Gestion Accès', icon: ShieldCheck, path: '/permissions-overview' },
      ];
        ...
        <div className="min-h-screen flex flex-col">
          <header className="glass rounded-b-3xl m-4 mb-0 animate-fade-in">
            <div className="container mx-auto px-6 py-6">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-4">
                  <span className="text-sm text-gray-700">Bienvenue, <span className="font-semibold">{currentUser?.name}</span></span>
                  <button
          ...
          <div className="flex flex-1">
            ...
            <main className="flex-1 px-6 py-8 min-w-0 overflow-x-auto w-full">
                  <Routes>
                    <Route path="/login" element={<Login />} />
                    <Route path="/dashboard" element={<ProtectedRoute><Dashboard key="dashboard-view" /></ProtectedRoute>} />
                    ...
                    <Route path="/checklists" element={<ProtectedRoute><PreDepartureChecklistComponent 
                      ...
                      onDelete={handleUpdateData}
                    /></ProtectedRoute>} />
                    <Route path="/reports" element={<ProtectedRoute><Reports key="reports-view" /></ProtectedRoute>} />
                    ...
                    {/* UserManagement is now accessible to all authenticated users */}
                    <Route path="/user-management" element={<ProtectedRoute><UserManagement 
                        key="user-management-view" 
                        // onUpdateUserRole={handleUpdateUserRole} // Removed prop
                      /></ProtectedRoute>} />
                    {/* PermissionsOverview is removed */}
                    ...
                    <Route path="*" element={<ProtectedRoute><Dashboard key="default-dashboard-view" /></ProtectedRoute>} />
                  </Routes>
            </main>
          </div>
        </div>
